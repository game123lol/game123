...
pub fn run_render_system_fov(
    ...
    let (_, (map,)) = query
        .iter()
        .next()
        .expect("Can't run render system without entity with Map component");

    let mut query = world.query::<(&Player, &Position, &Sight)>();
    let (_, (_, Position(cam_pos), Sight(sight_positions))) = query.iter().next().expect(
        "Can't run render system without entity with Player, Position and Sight components",
    );
    let (w, h) = canvas_size;
